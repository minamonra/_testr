...
// Коды ошибок
typedef enum { ERR_OK = 0, ERR_READ = -1, ERR_OUT = -2 } error_code_t;
...
#define EDTSTRLEN 14           // Длина редактируемой строки для отображения (16 символов + терминатор)
...
static const char messages[MSG_COUNT][32] = {[MSG_FUCK_OFF]         = "Отмена!",
                                             [MSG_WOW_NICE]         = "Сделал!",
...
int find_cp1251_index(char c) {
  for (int i = 0; cp1251_chars[i] != 0; i++) {
    if (cp1251_chars[i] == (unsigned char)c)
      return i;
  }
...
error_code_t cached_eeprom_read_string(uint8_t index, char *buf) {
  ...
  if (err == 0) {
    ...
    for (int i = 0; i < 32; i++) {
      if (buf[i] == '\0' || buf[i] == 0x00 || buf[i] == 0xFF)
        buf[i] = ' ';
    }
    buf[32] = '\0';
